define(['knockout', 'text!./scientific-literature.html', 'd3', 'jnj_chart', 'colorbrewer', 'lodash', 'knockout.dataTables.binding', 'jquery', 'jquery-ui', 'jquery-scrollTo'], function (ko, view, d3, jnj_chart, colorbrewer, _) {
	function scientificLiterature(params) {
			...
			*/
			...
			if (self.model.literatureEvidenceSummary().length <= 0) {
				...
				for (var i = 0; i < literatureEvidenceSummary.length; i++) {
					var literatureObject = {
						"label": literatureEvidenceSummary[i].evidence_type_suffix,
						"values": [literatureEvidenceSummary[i].evidence_count],
						"evidence_type": literatureEvidenceSummary[i].evidence_type
					};
		...
		self.literatureChartBarClick = function (evidenceType) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					for (var i=0; i<data.length; i++) {
						var id = data[i].sourceURL.match(regex);
						pubmedIds = pubmedIds.concat(id);
						if (i > 500) {
							break;
						}
					}
					var idList = pubmedIds.join('+');
					window.open('http://www.ncbi.nlm.nih.gov/pubmed/?term=' + idList);

					/*
					var returnVal = {
						"evidence_type": evidenceType,
						"evidence_count": data.length,
						"evidence": data
					};
		...
		self.renderBarChart = function (data, chartElement) {
			...
			bar.append("rect")
				...
				.attr("class", function (d, i) {
					return "bar " + data.series[i].evidence_type
				})
				.attr("width", x)
				.attr("height", barHeight - 1)
				.attr("evidence_type", function (d, i) {
					return data.series[i].evidence_type
				}) // { return literatureEvidenceSummary[i].evidence_type })
				.on('click', function (d) {
					self.literatureChartBarClick(this.attributes["evidence_type"].value);
				});
			...
			legend.append('rect')
				...
				.on('click', function (d) {
					self.literatureChartBarClick(d.evidence_type);
				});
